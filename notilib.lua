...
function CalculateBounds(TableOfObjects)
...
function Update()
	...
	for CurObj, Object in pairs(InstructionObjects) do
		...
		local NewValue = game:GetService("TweenService"):GetValue(Delta, TweenStyle, TweenDirection)
...
game:GetService("RunService"):BindToRenderStep("UpdateList", 0, Update)

function Label(Text, Font, Size, Button)
...
function FadeProperty(Object)
	local Prop = PropertyTweenOut[string.sub(Object.ClassName, 1, 4)]
	game:GetService("TweenService"):Create(Object, TweenInfo.new(0.25, TweenStyle, TweenDirection), {[Prop] = 1}):Play()
end

function SearchTableFor(Table, For)
...
function FindIndexByDependency(Table, Dependency)
...
function FadeOutAfter(Object, Seconds)
	wait(Seconds)
	FadeProperty(Object)
...
return {
	Notify = function(Properties)
		...
		if (Title) or (Description) then
			...
			if (Description) then
				local TextSize = game:GetService("TextService"):GetTextSize(Description, DescriptionSettings.Size, DescriptionSettings.Font, Vector2.new(0, 0))
